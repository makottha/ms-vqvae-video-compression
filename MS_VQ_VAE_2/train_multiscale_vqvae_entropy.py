    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for i, batch in enumerate(train_loader, 1):
            ...
            with autocast(device_type="cuda", enabled=USE_AMP):
                recon, vq_loss, avg_entropy, entropy_top, entropy_bottom, used_top, used_bottom = model(batch)
                loss, recon_l, vq_l, perc_l = vqvae2_loss(
            ...
            logger.log_train(epoch, i, loss.item(), recon_l.item(), vq_l.item(), perc_l)
            print(f"[TRAIN][BATCH {i}] Loss: {loss.item():.4f} | Recon: {recon_l.item():.4f} | VQ: {vq_l.item():.4f} | "
                  f"Perceptual: {perc_l:.4f} | Entropy (Top): {entropy_top:.4f} | Entropy (Bottom): {entropy_bottom:.4f}")
            print(f"[ENTROPY][BATCH {i}] Avg: {avg_entropy:.4f} | Used Codes - Top: {used_top}, Bottom: {used_bottom}")
        ...
        with torch.no_grad():
            for j, batch in enumerate(val_loader, 1):
                ...
                val_loss += loss.item()
                logger.log_val(epoch, j, loss.item(), recon_l.item(), vq_l.item(), perc_l)
